...
func FindAllPeople(n int, meetings [][]int, firstPerson int) []int {
	peopleWithSecret := []int{0, firstPerson}
	...
	for _, meet := range meetings {
		...
		if IsIncludes(peopleWithSecret, meet[0]) && !IsIncludes(peopleWithSecret, meet[1]) {
			peopleWithSecret = append(peopleWithSecret, meet[1])
		}
